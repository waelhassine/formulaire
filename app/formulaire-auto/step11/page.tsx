...
export default function FormulaireStep10() {
  ...
  const validateStep = async () => {
    ...
    if (result && isValid) {
      router.push('/formulaire-auto/step12');
    }
  };

  return (
    <div className="w-full">
      <ProgressHeader val={55} />
      <button
        className="flex flex-row space-x-2 items-center justify-center mt-6"
        onClick={() => router.push('step10')}
      >
      ...
      <div className="flex flex-col space-y-4 lg:w-2/3">
        ...
        <p className="flex flex-row text-2xl pt-12">Le conducteur a t il déclaré des sinistres ?</p>

        {fields.map((field, index) => (
          <Cardstep11 key={field.id} index={index} onClose={() => remove(index)} />
        ))}
